...
```
Shortcut for ligand-driven runs:
```bash
rinrus driver --pdb protein.pdb --ligand ligand.sdf --auto 4.0
```
...
```bash
rinrus driver --pdb protein.pdb --seed A:300 --rin-program distance --model max --output results/model_h.pdb
```
